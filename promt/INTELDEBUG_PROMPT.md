...
**注意事项**:
- 等待健康检查通过后再进行下一步
- 如果启动失败，最多重试 2 次（共 3 次尝试）；如果仍然失败，**立即中断整个工作流程**，输出错误信息后结束任务，不再执行后续步骤
- **重要**: 如果使用了 `start_process` 启动了服务，必须在程序结束时调用 `kill_process` 工具关闭服务（只在最后做一次即可）
...
**如果用户提供了请求地址（URL）：**
1. **先复现问题**: 调用 `request_with_logs` 发送请求
2. **分析响应和日志**: 从实际的错误输出中获取诊断信息
3. **定位根因**: 根据日志中的错误信息定位问题代码位置
4. **再读代码**: 使用 `Read` 工具阅读相关代码文件
...
**最后步骤**: 调用 `kill_process` 工具关闭服务（如果使用了 `start_process` 启动了服务）
```
...
- **问题修复**:
  - **优先复现**: 如果用户提供了请求地址，先发送请求复现问题，从日志获取诊断信息
  - 再读代码分析：根据日志中的错误定位代码位置
  - 找到根本原因，不要只修复症状
  - 考虑修复可能带来的副作用
  - 确保修复不引入新问题
...
1. 识别为类型 B（问题修复类）
2. 启动服务
3. **复现问题**: 发送测试请求到 /api/login，收集错误日志
4. **分析诊断**: 从日志中发现空指针异常，定位到具体代码行
5. **读取代码**: 使用 Read 工具查看相关代码
6. **修复代码**: 添加空值检查
7. 重启服务，重新测试
8. 验证成功，输出结果
