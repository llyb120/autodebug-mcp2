...
2. 需要查看之前的调试历史时
3. 任务重启需要了解之前进度时
4. **⚠️ 每次更新记忆之前**（必须先读取再修改）

**重要**：每次保存都要**累积**之前的步骤记录，不要丢失历史！

**⚠️ 更新记忆的强制流程**：
```
1. read_memory(memory_id) → 获取现有 content
...
**检索知识**：
```
search_knowledge(work_dir="D:/project/partner-ogdb-backend-intelligence-pc-backend-master ", query="超时", tags=["Go"])
search_knowledge(work_dir="D:/project/partner-ogdb-backend-intelligence-pc-backend-master ", category="问题解决")
search_knowledge(work_dir="D:/project/partner-ogdb-backend-intelligence-pc-backend-master ")  // 列出所有知识
```
